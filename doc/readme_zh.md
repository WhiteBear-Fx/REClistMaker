...
**用途**：根据音节词典生成录音表文件（如 `.txt`）， `oto.ini` 模板及 `presamp.ini`。
...
- **`-m, --mode`**（可选，默认 `CVVC`）  
  选择生成模式，可选 `VCV` 或 `CVVC`。

- **`-b, --bpm`**（可选，默认 `120`）  
  录音背景音乐的速度（BPM），用于生成节奏参考。

- **`-l, --max-length`**（可选，默认 `6`）  
  每行最多包含的音节数量，取值范围 2 到 8，用于排版录音表。

- **`-s, --SSS-first`**（可选，标志）  
  优先使用“三连重复音节”（Three Consecutive Repeat Syllable）模式排列，即优先构造连续三个相同音节的序列，这种模式通常具有较高的顺口性。

- **`-d, --iter-depth`**（可选）  
  寻找最优排列顺序时的最大迭代深度。增大此值可能找到更顺口的排列，但会增加计算时间。默认值和最大值均为 `-l` 参数的一半。

- **`-r, --max-redundancy`**（可选，默认 `50`）  
  允许为提升顺口性而额外添加的冗余音节数量上限。值越大，程序可插入更多多余音节来优化顺口性，但录音表也会相应变长。
